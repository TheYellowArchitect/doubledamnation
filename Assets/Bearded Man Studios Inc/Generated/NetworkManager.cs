...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkFinalCommunicationControllerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkFinalCommunicationControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkFinalCommunicationControllerNetworkObject.Length > 0 && NetworkFinalCommunicationControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkFinalCommunicationControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkFinalCommunicationControllerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkFinalCommunicationController instead, its shorter and easier to type out ;)")]
		public NetworkFinalCommunicationControllerBehavior InstantiateNetworkFinalCommunicationControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkFinalCommunicationControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkFinalCommunicationControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkFinalCommunicationControllerBehavior>().networkObject = (NetworkFinalCommunicationControllerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of NetworkFinalCommunicationController
		/// </summary>
		/// <returns>
		/// A local instance of NetworkFinalCommunicationControllerBehavior
		/// </returns>
		/// <param name="index">The index of the NetworkFinalCommunicationController prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NetworkFinalCommunicationControllerBehavior InstantiateNetworkFinalCommunicationController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (NetworkFinalCommunicationControllerNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: NetworkFinalCommunicationController. Add them at the NetworkManager prefab.");
				return null;
			}
